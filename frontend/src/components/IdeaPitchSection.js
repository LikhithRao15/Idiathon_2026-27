...
export default function IdeaPitchSection() {
  ...
  const handleCreateTeam = async (e) => {
    e.preventDefault();
    if (!selectedTheme) {
      showToast('Please select a focus waste theme', true);
      return;
    }
    ...
    if (res.ok) {
      showToast(`Team '${teamName}' successfully created!`);
      await loadMyTeam();
      setActiveTab('round1');
    } else {
      showToast(res.message || 'Failed to create team', true);
    }
  };

  const handleR1Submit = async (isDraft) => {
    if (!r1Problem.trim()) {
      showToast('Please provide a problem statement.', true);
      return;
    }
    if (!r1Solution.trim()) {
      showToast('Please describe your proposed solution.', true);
      return;
    }
    ...
    if (problemWords > 150) {
      showToast(`Problem statement exceeds 150 words (${problemWords} words).`, true);
      return;
    }
    if (solutionWords > 250) {
      showToast(`Proposed solution exceeds 250 words (${solutionWords} words).`, true);
      return;
    }
    ...
    if (res.ok) {
      showToast(isDraft ? 'Draft saved successfully!' : '🎉 Round 1 Idea Pitch submitted to MongoDB!');
      await loadMyTeam();
  ...
  const handleR2Submit = async () => {
    ...
    if (!r2Data.detailedConcept.trim() || !r2Data.valuePropositionAndCircularity.trim()) {
      showToast('Please fill out all required Round 2 sections.', true);
      return;
    }
    ...
    if (valPropWords > 150) {
      showToast(`Value Proposition exceeds 150 words limit (${valPropWords} words).`, true);
      return;
    }
    if (feasibilityWords > 200) {
      showToast(`90-Day Plan exceeds 200 words limit (${feasibilityWords} words).`, true);
      return;
    }
    if (resourceWords > 100) {
      showToast(`Resource Requirements exceeds 100 words limit (${resourceWords} words).`, true);
      return;
    }
    ...
    if (res.ok) {
      showToast('🎉 Round 2 Deep-Dive Dossier successfully submitted!');
      await loadMyTeam();
    ...
    <section id="pitch" className="section" style={{ background: '#ffffff', borderTop: '2px solid rgba(12, 91, 53, 0.15)' }}>
      <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <span className="section-tag">Team & Submissions</span>
            <h2 className="section-title" style={{ margin: 0, textAlign: 'left' }}>
              🌱 Idea Pitch & Evaluation Workspace
            </h2>
            <p style={{ color: 'var(--muted)', marginTop: '4px', fontSize: '14px' }}>
              Logged in as Team Leader: <strong>{user?.name}</strong> ({user?.email})
            </p>
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px', borderBottom: '2px solid rgba(12, 91, 53, 0.12)', marginBottom: '28px', overflowX: 'auto' }}>
          ...
          >
            ...
            {team ? 'My Team & Members' : 'Create Team'}
          </button>
          ...
          >
            ...
            Round 2: Deep-Dive Dossier {isR2Unlocked ? '🟢' : '🔒'}
          </button>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '18px', marginBottom: '24px' }}>
                  <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '16px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--green2)', textTransform: 'uppercase' }}>Designated Leader</div>
                    <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)', marginTop: '4px' }}>{user?.name}</div>
                    <div style={{ fontSize: '13px', color: 'var(--muted)' }}>{user?.email} • {user?.phone || 'No phone'}</div>
                  </div>

                  {team.members?.map((m, idx) => (
                    <div key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px' }}>
                      <div style={{ fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Teammate {idx + 1} ({m.roleInTeam || 'Member'})</div>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. EcoInnovators Hub"
                      value={teamName}
                  ...
                  <div className="form-group">
                    <label>Primary Waste Challenge Track *</label>
                    <select
                      required
                      value={selectedTheme}
                      onChange={(e) => setSelectedTheme(e.target.value)}
                    >
                      <option value="">-- Choose Focus Track --</option>
                      {themes.map((th) => (
                  ...
                  {members.map((m, idx) => (
                    <div key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--green2)' }}>Teammate #{idx + 1}</span>
          ...
          <div className="light-card" style={{ padding: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
              <div>
                <h3 style={{ fontSize: '20px', fontWeight: 800, color: 'var(--ink)', margin: 0 }}>
                  Round 1: Idea Pitch Submission
                </h3>
                <p style={{ color: 'var(--muted)', fontSize: '13px', marginTop: '4px' }}>
                  Deadline: <strong>Nov 1, 2026</strong> • Clear, concise problem definition & technical solution
                </p>
              </div>
            ...
            {!team ? (
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                <p style={{ color: 'var(--muted)', marginBottom: '16px' }}>Please create your team first to submit a Round 1 pitch.</p>
              ...
              <div>
                ...
                <div className="form-group" style={{ marginBottom: '24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      1. Problem Statement & Root Cause Brief *
                    </label>
                  ...
                  <textarea
                    rows={4}
                    disabled={isR1Locked}
                    placeholder="Clearly articulate the specific waste challenge, affected ecosystem/community, and why existing solutions fail..."
                    value={r1Problem}
                ...
                <div className="form-group" style={{ marginBottom: '28px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      2. Proposed Technical Architecture & Intervention *
                    </label>
                  ...
                  <textarea
                    rows={6}
                    disabled={isR1Locked}
                    placeholder="Describe your technical innovation, mechanical/chemical workflow, digital tools, and expected practical impact..."
                    value={r1Solution}
                ...
                {!isR1Locked ? (
                  <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                    ...
                    <button type="button" className="btn btn-primary" onClick={() => handleR1Submit(false)}>
                      <Send size={16} /> Final Submit Proposal
                    </button>
                  </div>
                ) : (
        ...
        {activeTab === 'round2' && (
          <div className="light-card" style={{ padding: '32px' }}>
            <div style={{ marginBottom: '20px' }}>
              <h3 style={{ fontSize: '20px', fontWeight: 800, color: 'var(--ink)', margin: 0 }}>
                Round 2: Concept Elaboration & 90-Day Feasibility Plan
              </h3>
              <p style={{ color: 'var(--muted)', fontSize: '13px', marginTop: '4px' }}>
                Timeline: <strong>Nov 2 – Dec 1, 2026</strong> • Strict access for teams marked SELECTED in Round 1
              </p>
            </div>
            ...
            {!isR2Unlocked ? (
              <div style={{ textAlign: 'center', padding: '50px 20px', background: '#f8fafc', borderRadius: '16px', border: '2px dashed #cbd5e1' }}>
                ...
                <h4 style={{ fontSize: '18px', fontWeight: 800, color: '#334155' }}>Round 2 Access Locked</h4>
                <p style={{ color: 'var(--muted)', maxWidth: '520px', margin: '8px auto 0 auto', fontSize: '14px' }}>
                  Round 2 is strictly unlocked once your Round 1 pitch is evaluated and marked <strong>SELECTED</strong> by the expert panel.
                </p>
              </div>
            ) : (
              <div>
                ...
                <div className="form-group" style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      1. Detailed Concept & Technical Specifications *
                    </label>
                  ...
                  <textarea
                    rows={6}
                    placeholder="Comprehensive technical architecture, components, workflows, and process flows..."
                ...
                <div className="form-group" style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      2. Value Proposition & Circularity Model *
                    </label>
                  ...
                  <textarea
                    rows={3}
                    placeholder="How does your solution transform waste into reusable value or resource loops..."
                ...
                <div className="form-group" style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      3. 90-Day Implementation & Pilot Feasibility Plan *
                    </label>
                  ...
                  <textarea
                    rows={4}
                    placeholder="Milestone roadmap: Month 1 (Lab validation), Month 2 (Pilot run), Month 3 (Community rollout)..."
                ...
                <div className="form-group" style={{ marginBottom: '28px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <label style={{ fontWeight: 700, color: 'var(--ink)' }}>
                      4. Resource Requirements & Budget Estimates *
                    </label>
                  ...
                  <textarea
                    rows={3}
                    placeholder="Estimated seed budget, hardware/cloud infrastructure needs, and mentors required..."
                ...
                <div style={{ textAlign: 'right' }}>
                  <button type="button" className="btn btn-primary" onClick={handleR2Submit}>
                    <Send size={16} /> Submit Round 2 Dossier
                  </button>
                </div>
              </div>
            )}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
              ...
              <div>
                ...
                <p style={{ color: '#b45309', margin: '4px 0 0 0', fontWeight: 600 }}>
                  Hasiru Samvadha Ideathon 2026 • Live Jury Demonstration
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '18px', marginBottom: '24px' }}>
              <div style={{ background: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #fde68a' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: '#b45309', textTransform: 'uppercase' }}>Event Date</div>
              ...
              <div style={{ background: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #fde68a' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: '#b45309', textTransform: 'uppercase' }}>Time Slot</div>
                <div style={{ fontSize: '16px', fontWeight: 800, color: '#78350f', marginTop: '4px' }}>
              ...
              <div style={{ background: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #fde68a' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: '#b45309', textTransform: 'uppercase' }}>Allocated Pitch Duration</div>
                <div style={{ fontSize: '16px', fontWeight: 800, color: '#78350f', marginTop: '4px' }}>
                  {finalistPass.presentationDuration || '15 Minutes'}
                </div>
              </div>

              <div style={{ background: '#ffffff', padding: '16px', borderRadius: '12px', border: '1px solid #fde68a' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: '#b45309', textTransform: 'uppercase' }}>Auditorium Stage / Venue</div>
                <div style={{ fontSize: '16px', fontWeight: 800, color: '#78350f', marginTop: '4px' }}>
                  {finalistPass.venue || 'Grand Innovation Auditorium'}
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#ffffff', padding: '18px 24px', borderRadius: '12px', border: '1px solid #fde68a' }}>
              <div style={{ fontSize: '12px', fontWeight: 800, color: '#b45309', marginBottom: '6px' }}>
                SPECIAL INSTRUCTIONS FOR PRESENTERS:
              </div>
              <p style={{ color: '#78350f', fontSize: '14px', margin: 0, lineHeight: 1.6 }}>
                {finalistPass.instructions || 'Please arrive 15 minutes before your scheduled presentation with your slides on a USB drive and working hardware demos.'}
              </p>
            </div>
          </div>
        )}
